...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // -------- Cuadrícula de categorías (una sola plantilla, datos desde Supabase) --------
  // categories-data.js pide las categorías a Supabase de forma asíncrona, así que
  // esta función se llama cuando el evento "talay:categories-ready" avisa que ya
  // están disponibles en window.TALAY_CATEGORIES (en vez de asumir que ya llegaron).
  var categoryGrid = document.getElementById('category-grid');

  // Las imágenes pueden ser una ruta local vieja (ej. "pic/categoria-1.jpg")
  ...
  function renderCategoryGrid() {
    ...
    var categories = window.TALAY_CATEGORIES || [];

    if (categories.length === 0) {
      categoryGrid.innerHTML = '<p class="cart-empty">No se pudieron cargar las categorías. Intenta recargar la página.</p>';
      return;
    }

    categoryGrid.innerHTML = categories.map(function (cat) {
      return (
        '<a href="categoria-productos.html?cat=' + encodeURIComponent(cat.id) + '" class="category-card">' +
          '<div class="category-media">' +
            '<img src="' + resolveImageSrc(cat.image) + '" alt="' + cat.name + '">' +
          '</div>' +
          '<div class="category-info">' +
            '<h3>' + cat.name + '</h3>' +
            '<p>' + cat.description + '</p>' +
            '<span class="category-link">Ver categoría →</span>' +
          '</div>' +
        '</a>'
      );
    }).join('');
  ...
  document.addEventListener('talay:categories-ready', renderCategoryGrid);
