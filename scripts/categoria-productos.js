...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // -------- Plantilla de "productos por categoría" --------
  // Lee el id de categoría desde la URL (?cat=), busca su info en
  // window.TALAY_CATEGORIES y filtra window.TALAY_PRODUCTS por categoryId.
  // Esta misma plantilla sirve para cualquier categoría: solo cambia el
  // parámetro ?cat= en el enlace que llega a esta página.
  //
  // Como ambos arreglos ahora se llenan de forma asíncrona (Supabase),
  // esperamos a que los DOS eventos "ready" hayan disparado antes de
  // buscar la categoría y filtrar los productos.
  var params = new URLSearchParams(window.location.search);
  var catId = params.get('cat');

  var catTitleEl = document.getElementById('cp-category-title');
  var catDescEl = document.getElementById('cp-category-desc');
  var catBreadcrumbEl = document.getElementById('cp-breadcrumb-current');
  var productGrid = document.getElementById('cp-product-grid');
  var emptyStateEl = document.getElementById('cp-empty-state');
  ...
  function renderCategoryProducts() {
    ...
    var allCategories = window.TALAY_CATEGORIES || [];
    var category = allCategories.find(function (c) { return c.id === catId; });

    if (category) {
      document.title = category.name + ' — Talay';
      if (catTitleEl) catTitleEl.textContent = category.name;
      if (catDescEl) catDescEl.textContent = category.description;
      if (catBreadcrumbEl) catBreadcrumbEl.textContent = category.name;
    } else if (catTitleEl) {
      catTitleEl.textContent = 'Categoría no encontrada';
      if (catDescEl) catDescEl.textContent = 'Elige una categoría desde la página de Categorías.';
    }

    var categoryProducts = catId ? allProducts.filter(function (p) { return p.categoryId === catId; }) : [];
    ...
    productGrid.innerHTML = categoryProducts.map(function (product) {
      return (
  ...
  function tryRenderCategoryProducts() {
    if (productsReady && categoriesReady) renderCategoryProducts();
  }
  document.addEventListener('talay:products-ready', function () { productsReady = true; tryRenderCategoryProducts(); });
  document.addEventListener('talay:categories-ready', function () { categoriesReady = true; tryRenderCategoryProducts(); });
