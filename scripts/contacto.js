...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Formulario de contacto (placeholder): evita el envío real y confirma en pantalla
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', async function (e) {
      ...
      try {
        // FormSubmit.co envía el contenido del formulario directo a este correo,
        // sin necesitar backend propio. La primera vez, ese correo debe confirmar
        // un email de activación (una sola vez).
        var response = await fetch('https://formsubmit.co/ajax/talayshop.mx@gmail.com', {
          method: 'POST',
        ...
        if (formNote) {
          formNote.textContent = '¡Gracias! Tu mensaje fue enviado, te responderemos pronto.';
          formNote.className = 'form-note success';
        }
        contactForm.reset();
      } catch (err) {
        if (formNote) {
          formNote.textContent = 'No se pudo enviar tu mensaje. Intenta de nuevo o escríbenos directo a talayshop.mx@gmail.com.';
